...
SELECT * FROM AUTHORISEDPERSON3778
SELECT * FROM ORDERLINE3778
SELECT * FROM INVENTORY3778
SELECT * FROM ORDER3778
SELECT * FROM CLIENTACCOUNT3778
SELECT * FROM GENERALLEDGER3778
SELECT * FROM ACCOUNTPAYMENT3778
SELECT * FROM PURCHASEORDER3778 
SELECT * FROM LOCATION3778
SELECT * FROM PRODUCT3778
...
BEGIN
    BEGIN TRY
                ...
                ELSE
                    SELECT * FROM LOCATION3778 WHERE LOCATIONID LIKE @PLOCID
    END TRY
    BEGIN CATCH
...
BEGIN
    BEGIN TRY
        IF EXISTS(SELECT * FROM PRODUCT3778 WHERE PRODUCTID = @PPRODID)
            SELECT * FROM PRODUCT3778 WHERE PRODUCTID = @PPRODID
        ELSE
...
BEGIN
    BEGIN TRAN
        BEGIN TRY
            ...
            UPDATE GENERALLEDGER3778 SET AMOUNT = AMOUNT + (@PQty * @BuyP) WHERE ITEMID = 2;
...
BEGIN
    BEGIN TRY
        IF EXISTS(SELECT * FROM CLIENTACCOUNT3778 WHERE ACCOUNTID = @PACCOUNTID)
            BEGIN
                IF EXISTS(SELECT * FROM AUTHORISEDPERSON3778 WHERE ACCOUNTID = @PACCOUNTID)
                    ...
                    WHERE CLIENTACCOUNT3778.ACCOUNTID = @PACCOUNTID
                ELSE
                    SELECT ACCOUNTID, ACCTNAME, BALANCE, CREDITLIMIT, 
                    (SELECT NULL) AS USERID, (SELECT 'null') AS FIRSTNAME, (SELECT 'null') AS SURNAME, (SELECT 'null') AS EMAIL
                    FROM CLIENTACCOUNT3778 WHERE ACCOUNTID = @PACCOUNTID 
...
BEGIN
    BEGIN TRY
        IF EXISTS(SELECT * FROM ORDER3778 WHERE ORDERID = @PORDERID)
            BEGIN
                IF EXISTS(SELECT * FROM ORDERLINE3778 WHERE ORDERID = @PORDERID)
                ...
                ELSE
                    SELECT ORDER3778.*,  (SELECT NULL) AS PRODUCTID, (SELECT NULL) AS QUANTITY, 
                    (SELECT NULL) AS DISCOUNT, (SELECT NULL) AS SUBTOTAL
                    FROM ORDER3778
...
BEGIN
    BEGIN TRAN
        BEGIN TRY
            ...
            DECLARE @SubTotal MONEY = (SELECT SUBTOTAL FROM ORDERLINE3778
            WHERE ORDERID = @PORDERID AND PRODUCTID = @PPRODIID)
...
BEGIN
    BEGIN TRY
        SELECT * FROM ORDER3778 WHERE DATETIMEDISPATCHED IS NULL 
    END TRY
    BEGIN CATCH              
        DECLARE @ERRORMESSAGE NVARCHAR(MAX) = ERROR_MESSAGE();          
        THROW 50000, @ERRORMESSAGE, 1    
